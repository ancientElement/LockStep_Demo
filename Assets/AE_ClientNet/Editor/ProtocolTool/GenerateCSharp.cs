...
namespace AE_ClientNet
{
    public class GenerateCSharp
    {
            ...
            foreach (XmlNode messageNode in nodeList)
            {
                ...
                if (systemMessage != null && systemMessage == "1")
                {
                    ...
                    messageStr = $"namespace {namespaceStr}" +
                                    "{\r\n" +
                                        $"public class {classNameStr} : AE_NetMessage.BaseSystemMessage" +
                                         "{\r\n" +
                                           $"{GetMessageIDMethod}" +
                                           "\r\n}" +
                                    "\r\n}";
                ...
                else
                {
                    ...
                    messageStr = $"namespace {namespaceStr}" +
                                    "{\r\n" +
                                        $"public class {classNameStr} : AE_NetMessage.BaseMessage<{dataType}>" +
                                         "{\r\n" +
                                           $"{GetMessageIDMethod}" +
                                           $"{WriteInMethod}" +
                                           "\r\n}" +
        ...
        public static void GenerateMessagePool(XmlNodeList nodeList)
        {
            ...
            string MessageTypeMap = " private static readonly System.Collections.Generic.Dictionary<int, System.Func<AE_NetMessage.BaseMessage>> MessageTypeMap = new System.Collections.Generic.Dictionary<int, System.Func<AE_NetMessage.BaseMessage>>\r\n        {\r\n";

            string GetMessage = "public static AE_NetMessage.BaseMessage GetMessage(int id) " +
                                "{" +
                                "       if (MessageTypeMap.TryGetValue(id, out System.Func<AE_NetMessage.BaseMessage> messageFactory)) {  " +
